📚 Direito Ambiental na Palma da Mão
Um caderno inteligente de Direito Ambiental desenvolvido com o Gemini NotebookLM para reunir legislação, documentos oficiais, resumos e prompts de estudo em um único ambiente de aprendizagem.

📖 Sobre o Projeto
Este repositório foi desenvolvido como parte de um desafio da Digital Innovation One (DIO), explorando o uso da Inteligência Artificial como ferramenta de aprendizagem ativa.

O projeto tem como finalidade construir um Caderno Inteligente de Direito Ambiental utilizando o Google NotebookLM, reunindo fontes oficiais, legislação ambiental brasileira, documentos técnicos, resumos estruturados e prompts reutilizáveis para facilitar os estudos e a consulta rápida de informações confiáveis.

Além de atender ao desafio proposto, este repositório foi concebido para evoluir como uma biblioteca digital de apoio aos estudos em Direito Ambiental.

🎯 Objetivos
Reunir a legislação ambiental brasileira em um único local.
Centralizar fontes oficiais de consulta.
Organizar materiais de estudo de forma estruturada.
Utilizar Inteligência Artificial para aprofundar a aprendizagem.
Criar um conjunto de prompts reutilizáveis.
Desenvolver um guia permanente para estudos e revisões.
👥 Público-alvo
Estudantes
Biólogos
Engenheiros Ambientais
Gestores Ambientais
Peritos Ambientais
Advogados
Servidores públicos
Candidatos a concursos
Pesquisadores da área ambiental
📂 Estrutura do Repositório
Direito-ambiental-na-palma-da-mao
│
├── Legislacao
├── Fontes
├── Prompts
├── Glossario
├── Resumos
└── README.md
📚 Fontes Oficiais
Neste projeto serão utilizadas exclusivamente fontes oficiais, tais como:

Constituição Federal
Portal da Legislação (Presidência da República)
Ministério do Meio Ambiente e Mudança do Clima
Instituto Brasileiro do Meio Ambiente e dos Recursos Naturais Renováveis (IBAMA)
Instituto Chico Mendes de Conservação da Biodiversidade (ICMBio)
Conselho Nacional do Meio Ambiente (CONAMA)
🤖 Engenharia de Prompts
Durante o desenvolvimento do caderno serão registrados:

Prompts utilizados.
Variações testadas.
Estratégias de refinamento.
Dificuldades encontradas.
Soluções adotadas.
Lições aprendidas.
📖 Miniguia de Estudos
Ao final do projeto, este repositório reunirá:

Resumos estruturados.
Glossário de termos.
Perguntas para revisão.
Prompts reutilizáveis.
Referências bibliográficas.
🚧 Status do Projeto
Em desenvolvimento.

Novas legislações, resumos e materiais serão adicionados continuamente.

🛠️ Ferramentas Utilizadas
GitHub
Git
Gemini NotebookLM
Gemini
Markdown

📄 Licença
Este projeto está licenciado sob a Licença MIT.

Os conteúdos produzidos neste repositório (resumos, organização, prompts e documentação) seguem essa licença.

Os textos legais, normas e documentos oficiais citados permanecem sujeitos às regras de publicação dos respectivos órgãos governamentais e são utilizados apenas para fins educacionais e de consulta.
